...
class JPKTDocumentType(models.Model):
    ...
    description = fields.Text(size=250)
    ...
    @api.constrains('system_code', 'name')
    def constrains_unique_values(self):
        for record in self:
            ...
            if len(self.search([('system_code', '=', record.system_code)])) > 1:
                raise ValidationError(_('System Code must be unique'))
